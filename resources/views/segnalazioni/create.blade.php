        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('segnalazioni.store') }}" class="space-y-6" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div>
                            ...
                            <select id="tipo" name="tipo" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                ...
                                <option value="Rifiuti abbandonati">Rifiuti abbandonati</option>
                                <option value="Cassonetti danneggiati">Cassonetti danneggiati</option>
                                <option value="Mancato ritiro">Mancato ritiro</option>
                        ...
                        <div>
                            ...
                            <textarea id="descrizione" name="descrizione" rows="4" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm"></textarea>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div class="col-span-1 md:col-span-2">
                                ...
                                <x-text-input id="via" name="via" type="text" class="mt-1 block w-full" required />
                                <x-input-error :messages="$errors->get('via')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="civico" :value="__('Numero Civico')" />
                                <x-text-input id="civico" name="civico" type="text" class="mt-1 block w-full" required />
                                <x-input-error :messages="$errors->get('civico')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="cap" :value="__('CAP')" />
                                <x-text-input id="cap" name="cap" type="text" class="mt-1 block w-full" required />
